...
- 100 曲上限のキュー、再生ごとに 1 曲追加、10 曲超過で自動トリム
- コメントのページネーション読み込み（20 件ずつ、スクロールで追加）
- OPPO Fluid Cloud リアルタイム通知：アートワーク、進行状況、歌詞、再生/一時停止/スキップ操作
- 単曲リピート、全曲リピート、順次再生、シャッフルの 4 モード
