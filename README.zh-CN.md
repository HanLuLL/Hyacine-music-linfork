...
- 中心歌词、全幅封面、紧凑封面、封面与歌词四种播放器布局
- 网易云扫码登录、个人资料、每日推荐、推荐歌单、我的歌单、搜索、播放、歌词和只读评论
- 本地收藏与听歌历史
- 收藏同步到网易云专属歌单「收藏风堇音乐」，未登录或同步失败时本地仍保留
- 播放历史或收藏歌曲时重新获取播放地址，避免旧流地址失效
- 歌词自动定位，点击歌词跳转播放时间
- 独立音质与音效分类：网易云在线播放音质、音效预设和十段自定义均衡器配置
- 只有底部导航栏响应左右滑动切页，内容区不接管切页手势
- 管理后台：查看当前设备用户摘要、历史/收藏数量、音乐源凭据是否存在、脱敏日志与后端健康状态
- 播放队列支持顺序、列表循环、随机三种模式（单一按钮循环切换）
- 播放完成自动切下一首（不依赖当前所在页面）
- 锁屏媒体控制：标题、歌手、封面、前后跳转
- 管理后台与队列页隐藏 MiniPlayer，浏览更干净
- 每日推荐卡片原生滑动手势
- 推荐歌单分页加载
- 后端健康监控，非 JSON 响应自动保护
- 桌面歌词浮窗支持
- 单曲循环模式重新播放当前歌曲，不再误切下一首
- OPPO/realme 设备的流体云正在播放卡片显示封面、标题、歌手、歌词、实时进度与播放/暂停/上一首/下一首/拖动进度操作
