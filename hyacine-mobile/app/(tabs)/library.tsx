  ...
  const loadPlaylists = useCallback(async (refresh = false): Promise<void> => {
    ...
    try {
      if (!(await supportsNeteaseCapability(profile.backendUrl, "playlists"))) {
        setPlaylists([]);
        setError(t("playlistsServerUnsupported"));
        return;
      }
      ...
      if (!response.ok || !Array.isArray(data)) {
        throw new Error((data as { message?: string }).message || t("loadFailed"));
      }
      setPlaylists(data);
      ...
      setPlaylists([]);
      setError(cause instanceof Error ? cause.message : t("loadPlaylistsFailed"));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
...
Alert.alert(t("deletePlaylist"), t("deleteConfirm").replace("{name}", playlist.name), [
      ...
      { text: t("delete"), style: "destructive", onPress: () => void (async () => {
        try {
          ...
          if (!response.ok) throw new Error(result.message || t("deleteFailed"));
          setPlaylists((current) => current.filter((item) => item.id !== playlist.id));
        } catch (cause) { setError(cause instanceof Error ? cause.message : t("deleteFailed")); }
      })() },
    ]);
  };
  ...
  return <ThemedScreen><ScrollView className="flex-1" contentContainerClassName="px-5 pb-40 pt-16" onScroll={({ nativeEvent }) => { const { contentOffset, contentSize, layoutMeasurement } = nativeEvent; if (contentOffset.y + layoutMeasurement.height >= contentSize.height - 300) void loadMoreRecommendations(); }} scrollEventThrottle={200} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void loadPlaylists(true)} tintColor={tokens.accent} />}>
    <View className="flex-row items-start justify-between"><View><Text style={{ color: tokens.text, fontSize: 31, fontWeight: "800" }}>{t("library")}</Text><Text className="mt-2 text-sm" style={{ color: tokens.mutedText }}>t("neteaseCloud")</Text></View><LiquidControlSurface className="h-11 w-11 items-center justify-center rounded-full"><Pressable className="h-full w-full items-center justify-center" onPress={() => void loadPlaylists(true)}><Text style={{ color: tokens.text, fontSize: 18 }}>↻</Text></Pressable></LiquidControlSurface></View>
    <LiquidControlSurface className="mt-10 flex-row overflow-hidden rounded-full p-1" style={{ borderRadius: 999 }}><Pressable className="h-11 flex-1 items-center justify-center rounded-full" style={{ backgroundColor: activeTab === "mine" ? `${tokens.accent}30` : "transparent" }} onPress={() => setActiveTab("mine")}><Text style={{ color: activeTab === "mine" ? tokens.text : tokens.mutedText, fontWeight: "800" }}>{t("myPlaylistsTab")}</Text></Pressable><Pressable className="h-11 flex-1 items-center justify-center rounded-full" style={{ backgroundColor: activeTab === "recommended" ? `${tokens.accent}30` : "transparent" }} onPress={() => setActiveTab("recommended")}><Text style={{ color: activeTab === "recommended" ? tokens.text : tokens.mutedText, fontWeight: "800" }}>{t("recommendedTab")}</Text></Pressable></LiquidControlSurface>
    {activeTab === "mine" ? <View className="mt-7 flex-row items-center justify-between"><Text style={{ color: tokens.text, fontSize: 21, fontWeight: "800" }}>{t("myPlaylists")}</Text><Pressable onPress={() => setShowCreate((value) => !value)}><Text className="text-xs font-semibold" style={{ color: tokens.accent }}>{showCreate ? t("cancelCreate") : t("createPlaylist")}</Text></Pressable></View> : <Text className="mt-7" style={{ color: tokens.text, fontSize: 21, fontWeight: "800" }}>{t("recommendedTab")}</Text>}
