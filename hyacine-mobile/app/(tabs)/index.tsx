...
function greetingForHour(hour: number, t: (key: string) => string): string {
  if (hour < 6) return t("greetingLateNight");
...
export default function HomeScreen(): React.JSX.Element {
  ...
  const [greeting, setGreeting] = useState(() => greetingForHour(new Date().getHours(), t));
  ...
  useEffect(() => {
    const timer = setInterval(() => setGreeting(greetingForHour(new Date().getHours(), t)), 60_000);
  ...
  const loadDailySongs = useCallback(async (): Promise<void> => {
    ...
    try {
      if (!(await supportsNeteaseCapability(profile.backendUrl, "dailySongs"))) {
        if (seq === requestSeq.current) {
          setSongs([]);
          setError(t("dailySongsUnavailable"));
        }
      ...
      if (!cookie) throw new Error(t("neteaseExpired"));
      ...
      if (!response.ok || !Array.isArray(data)) throw new Error((data as { message?: string }).message || t("loadFailed"));
  ...
  const onPlay = async (track: Track): Promise<void> => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t("playFailed"));
...
<Animated.View className="overflow-hidden p-5" style={{ opacity: recommendationEntrance, transform: [{ translateY: recommendationEntrance.interpolate({ inputRange: [0, 1], outputRange: [10, 0] }) }] }}>
             <GestureDetector gesture={recommendationSwipe}><View className="min-h-48 flex-row items-end">
              <View className="min-w-0 flex-1 pr-4">
                <Text className="text-xs font-bold" style={{ color: tokens.accent }}>{t("featured")}</Text>
            ...
            </GestureDetector><Pressable onPress={() => void onPlay(featured)}><LiquidControlSurface className="mt-5 h-12 self-start rounded-full px-5" style={{ borderRadius: 24 }}><View className="h-full flex-row items-center justify-center"><Text style={{ color: tokens.text, fontWeight: "800" }}>{playingId === featured.id ? "t("playing")" : "t("playFeatured")"}</Text></View></LiquidControlSurface></Pressable>
          </Animated.View>
