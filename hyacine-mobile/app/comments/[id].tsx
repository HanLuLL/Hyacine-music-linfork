...
export default function CommentsScreen(): React.JSX.Element {
  ...
  return <ThemedScreen><View className="flex-1 px-5 pb-8 pt-14">
    <View className="mb-5 flex-row items-center justify-between"><Pressable onPress={() => router.back()}><Text style={{ color: tokens.accent, fontWeight: "900" }}>‹ {t("goBack")}</Text></Pressable><View className="items-end"><Text numberOfLines={1} style={{ color: tokens.text, fontSize: 18, fontWeight: "900" }}>{params.title || t("comments")}</Text><Text style={{ color: tokens.mutedText, fontSize: 12 }}>{total} {t("commentsTotal")}</Text></View></View>
    ...
    {error ? <LiquidControlSurface className="p-4" style={{ borderRadius: 20 }}><Text style={{ color: tokens.text }}>{error}</Text><Pressable className="mt-3" onPress={() => void load()}><Text style={{ color: tokens.accent, fontWeight: "800" }}>{t("reload")}</Text></Pressable></LiquidControlSurface> : null}
    {!loading && !error && !comments.length ? <View className="flex-1 items-center justify-center"><Text style={{ color: tokens.mutedText }}>{t("noComments")}</Text></View> : null}
