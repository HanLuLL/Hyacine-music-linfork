...
function Controls({ playing, toggle, skip, canSkip, shuffleEnabled, toggleShuffle }: { playing: boolean; toggle: () => void; skip: (direction: -1 | 1) => void; canSkip: boolean; shuffleEnabled: boolean; toggleShuffle: () => void }): React.JSX.Element {
  ...
  return <LiquidControlSurface className="mt-6 self-center rounded-full px-3 py-2" style={{ borderRadius: 999 }}>
    <View className="flex-row items-center gap-3">
      <Pressable accessibilityLabel="随机播放" disabled={!canSkip} className="h-11 w-11 items-center justify-center" style={{ opacity: canSkip ? 1 : 0.35 }} onPress={toggleShuffle}><Text style={{ color: shuffleEnabled ? tokens.accent : tokens.text, fontSize: 22, fontWeight: "900" }}>⇄</Text></Pressable>
      <Pressable accessibilityLabel="上一首" disabled={!canSkip} className="h-11 w-11 items-center justify-center" style={{ opacity: canSkip ? 1 : 0.35 }} onPress={() => skip(-1)}><Text style={buttonStyle}>|◀</Text></Pressable>
      ...
      <Pressable accessibilityLabel="下一首" disabled={!canSkip} className="h-11 w-11 items-center justify-center" style={{ opacity: canSkip ? 1 : 0.35 }} onPress={() => skip(1)}><Text style={buttonStyle}>▶|</Text></Pressable>
    </View>
