...
function Controls({ playing, toggle, skip, canSkip, playMode, cyclePlayMode }: { playing: boolean; toggle: () => void; skip: (direction: -1 | 1) => void; canSkip: boolean; playMode: "sequential" | "loop" | "shuffle"; cyclePlayMode: () => void }): React.JSX.Element {
  const { tokens } = useTheme();
  const buttonStyle = { color: tokens.text, fontSize: 25, fontWeight: "900" as const };
  const modeIcon = playMode === "shuffle" ? "RAND" : playMode === "loop" ? "LOOP" : "SEQ";
  const modeLabel = playMode === "shuffle" ? "随机" : playMode === "loop" ? "循环" : "顺序";
  return <LiquidControlSurface className="mt-6 self-center rounded-full px-3 py-2" style={{ borderRadius: 999 }}>
    <View className="flex-row items-center gap-3">
      <Pressable accessibilityLabel={modeLabel} className="h-11 w-11 items-center justify-center" onPress={cyclePlayMode}><Text style={{ color: playMode !== "sequential" ? tokens.accent : tokens.text, fontSize: 22, fontWeight: "900" }}>{modeIcon}</Text></Pressable>
