...
function Controls({ playing, toggle, skip, canSkip, shuffleEnabled, toggleShuffle, repeatMode, cycleRepeat }: { playing: boolean; toggle: () => void; skip: (direction: -1 | 1) => void; canSkip: boolean; shuffleEnabled: boolean; toggleShuffle: () => void; repeatMode: "none" | "one" | "all"; cycleRepeat: () => void }): React.JSX.Element {
  ...
  return <LiquidControlSurface className="mt-6 self-center rounded-full px-3 py-2" style={{ borderRadius: 999 }}>
    <View className="flex-row items-center gap-3">
      ...
      <Pressable accessibilityLabel="循环模式" className="h-11 w-11 items-center justify-center" onPress={cycleRepeat}><Text style={{ color: repeatMode !== "none" ? tokens.accent : tokens.text, fontSize: 20, fontWeight: "900" }}>{repeatIcon}</Text></Pressable>
    </View>
