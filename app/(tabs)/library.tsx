...
export default function LibraryScreen(): React.JSX.Element {
  ...
  return <ThemedScreen><ScrollView className="flex-1" contentContainerClassName="px-5 pb-24 pt-16" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void loadPlaylists(true)} tintColor={tokens.accent} />}>
    ...
    {!loading && playlists.length ? <View className="mt-5 flex-row flex-wrap justify-between">{playlists.map((playlist) => <Pressable key={playlist.id} className="mb-7 w-[48%]" onPress={() => router.push({ pathname: '/playlist/[id]', params: { id: String(playlist.id), name: playlist.name, cover: normalizeCoverUrl(playlist.coverUrl) || '' } })}><View className="aspect-square overflow-hidden rounded-2xl" style={{ backgroundColor: tokens.surface }}><Image className="h-full w-full" source={{ uri: normalizeCoverUrl(playlist.coverUrl) }} contentFit="cover" /><View className="absolute bottom-2 right-2 rounded-full px-2 py-1" style={{ backgroundColor: "#00000080" }}><Text style={{ color: "#ffffff", fontSize: 10, fontWeight: "800" }}>▶ {formatPlayCount(playlist.playCount, language)}</Text></View></View><Text className="mt-3 text-sm font-bold" numberOfLines={2} style={{ color: tokens.text }}>{playlist.name}</Text><Text className="mt-1 text-xs" numberOfLines={1} style={{ color: tokens.mutedText }}>{playlist.description || `${playlist.trackCount} ${t("playlistTracks")}`}</Text></Pressable>)}</View> : null}
